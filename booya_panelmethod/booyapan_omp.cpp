...
int main(int argc, char* argv[]){
	...
	if (argc < 3) { // more than 2 arguments, or do nothing.
		...
		while(inputfile.getline(thisline, max_linelength)) {
			...
			panel->sigma = panel->norm_vect.dot(Eigen::Vector3d(12.,0.,2.));
		...
		Eigen::VectorXf rhs[2](num_not_wake);
			...
			for (int i = 0; i<num_panels; i++){//ROWS
				...
				for(int j=0; j<num_panels; j++){//COLUMNS
					...
					if (panel_i->wake){ //wake -> two neighbours (CHECK SIGNS!! maybe sign(dot(normvecs))))
						...
						if (panel_i->neighbour_front < 1 || panel_i->neighbour_front > num_panels || (panels + panel_i->neighbour_front - 1)->wake)
							cout << "Error in panel " << i << ": neighbour front not right!" << endl;
						if (panel_i->neighbour_back < 1 || panel_i->neighbour_back > num_panels || (panels + panel_i->neighbour_back - 1)->wake)
							cout << "Error in panel " << i << ": neighbour back not right!" << endl;
						matrix((panels + panel_i->neighbour_front -1)->position, panel_j->position)+=cjk;
						matrix((panels + panel_i->neighbour_back - 1)->position, panel_j->position)-=cjk;
						}
						...
						rhs(panel_j->position) += bjk*panel->sigma;
						}
